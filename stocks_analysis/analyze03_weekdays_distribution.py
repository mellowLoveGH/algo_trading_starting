...
def percentiles(tmp_df):
    r = round(tmp_df.mean()*100, 2)
    print( "\tmean: \t\t", r )
    r = round(tmp_df.median()*100, 2)
    print( "\tmedian: \t", r )
    for p in [0.1, 0.25, 0.5, 0.75, 0.9]:
        r = round(tmp_df.quantile(p)*100, 2)
        print( "\tpercentile \t", round(p*100), "%:", r )
    return 
...
for it in stocks_info[:]:
    ...
    nn_days = 160
    recent_data = df_data1[-nn_days:]
    print( ref_code + "_" + stock_code, "\t rise vs fall:" )
    for weekday in range(5):
        weekday_df = recent_data[ recent_data['weekday']==weekday ]
        rise_num = len( weekday_df[weekday_df['daily_return']>0] )
        fall_num = len( weekday_df[weekday_df['daily_return']<0] )
        L = rise_num + fall_num
        r1 = round(rise_num/L*100, 2)
        r2 = round(fall_num/L*100, 2)
        print( "weekday: ", weekday_dic[weekday] )
        print( "rise_num:\t", rise_num, "\t", r1 )
        print( "fall_num:\t", fall_num, "\t", r2 )
        #
        rise_df = weekday_df[weekday_df['daily_return']>0]
        print("for rise distribution: ")
        percentiles(rise_df['daily_return'])
        fall_df = weekday_df[weekday_df['daily_return']<0]
        print("for fall distribution: ")
        percentiles(fall_df['daily_return'])
    print()
