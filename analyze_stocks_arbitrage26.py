...
def merge_stocks(df_data1, df_data2):
    ...
    while i<len(df_data1):
        ...
        if ref_time<target_time:
            ...
            tmp_list = [ target_time, open1, high1, low1, close1, daily_return1, ref_time, open2, high2, low2, close2, daily_return2 ]
            data.append( tmp_list )
            #print( target_time, ref_time )
        i += 1
    col_names = ['target_time', 'target_open', 'target_high', 'target_low', 'target_close', 'target_return',
                'ref_time', 'ref_open', 'ref_high', 'ref_low', 'ref_close', 'ref_return']
...
tmp_df['previous_Close'] = real_df['previous_Close']
tmp_df['daily_return%'] = (tmp_df['close_real'] - tmp_df['previous_Close'])/tmp_df['previous_Close']*100
print_df = tmp_df.round({'close_predicted': 2, 'close_error%': 2, 'close_real': 2, 'dif%': 2, 'previous_Close':2, 'daily_return%':2 })
print( print_df )
